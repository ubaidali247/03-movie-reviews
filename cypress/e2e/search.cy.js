// Search and Filter Tests - Movie Reviews
describe('Search and Filter - Movie Reviews', () => {
  beforeEach(() => {
    cy.visit('/');
  ...
  it('searching for existing item title shows result', () => {
    cy.get('#movies-list .item-card').first().find('.item-title').invoke('text').then(title => {
  ...
  it('searching for non-existent term shows empty state', () => {
    ...
    cy.get('#movies-list').should('contain', 'No items found');
  });
  ...
  it('clearing search restores full list', () => {
    cy.get('#movies-list .item-card').then($cards => {
      ...
      cy.get('#movies-list .item-card').should('have.length', total);
    });
  });
  ...
  it('filter by Action category works', () => {
    cy.get('#filter-category').select('Action');
  ...
  it('api search endpoint filters results', () => {
    cy.request('/api/movies?search=test').its('body').should('be.an', 'array');
  });

  it('api category filter endpoint works', () => {
    cy.request('/api/movies?category=Action').its('body').should('be.an', 'array');
  });
  ...
  it('api returns all items when no filter applied', () => {
    cy.request('/api/movies').its('body').should('have.length.gte', 1);
  });
