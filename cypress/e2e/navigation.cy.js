// Navigation Tests - Movie Reviews
describe('Navigation - Movie Reviews', () => {
  beforeEach(() => {
    cy.visit('/');
  });
  ...
  it('loads the homepage successfully', () => {
    ...
    cy.get('.nav-brand').should('contain', 'Movie Reviews');
  });
  ...
  it('navigates to movies list page', () => {
  ...
  it('cancel button returns to movies list', () => {
    cy.get('.nav-link[data-page="add"]').click();
    cy.get('#btn-cancel').click();
    cy.get('#page-movies').should('not.have.class', 'hidden');
  });

  it('shows add new button on movies list page', () => {
    cy.get('.nav-link[data-page="movies"]').click();
  ...
  it('api health endpoint returns correct project name', () => {
    cy.request('/api/health').its('body.project').should('eq', 'Movie Reviews');
  });
});
