...
app.get('/api/movies', (req, res) => {
  ...
  if (shouldBeFlaky(FLAKY_CONFIG.slowProbability)) {
    ...
    console.log(`[FLAKY] Slow GET /api/movies +${delay}ms`);
...
app.post('/api/movies', (req, res) => {
  if (shouldBeFlaky(FLAKY_CONFIG.errorProbability)) {
    console.log(`[FLAKY] 500 error on POST /api/movies`);
...
app.listen(PORT, () => console.log('Movie Reviews running on http://localhost:3003 [FLAKY v2]'));
